...
if not os.path.exists('/etc/ckan/default/production.ini'):
    ...
    call(["/usr/lib/ckan/default/bin/pip", 'install', '-e', 'git+https://github.com/okfn/datapusher.git#egg=datapusher' ])
    call(["/usr/lib/ckan/default/bin/pip", 'install', '-e', 'git+https://github.com/okfn/ckanext-datapusherext.git#egg=ckanext-datapusherext' ])
    ports = open('/etc/apache2/ports.conf', 'a')
    ports.write('NameVirtualHost *:8800\n')
    ports.write('Listen 8800\n')
    ports.close()
    ...
    call(['sed', '-i', 's/^#ofs.storage.*/ofs.storage_dir = \/var\/lib\/ckan\/default/g', '/etc/ckan/default/production.ini'])
    call(['sed', '-i', r's/^ckan.plugins.*/ckan.plugins = stats text_preview recline_preview datastore datapusherext\n\nckan.datapusher.url = http:\/\/0.0.0.0:8800\//g', '/etc/ckan/default/production.ini'])
